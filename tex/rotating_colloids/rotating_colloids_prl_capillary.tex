...
matter, rearranged particles, hysteretic elements, defects, or fixed alignment
fields commonly carry that link \cite{keim2019}. Cyclically sheared
suspensions reorganize until their trajectories encode the training amplitude
\cite{keim2011,paulsen2014}. Quenched-disorder nematics instead retain local
easy axes \cite{rotunno2005}. Both mechanisms place the record outside a
freely evolving orientation. Colloidal ellipsoids provide a different
contrast: their orientations can arrest while their positions remain fluid
\cite{zheng2011,roller2021}. Slow rotation alone, however, does not identify
which microscopic pattern was prepared.

The unresolved question is whether an unchanged network of pair interactions
can retain a specified angular pattern after its writing field disappears.
Here we test that possibility in caged interfacial ellipsoids. A stored pattern
must remain identifiable after release and must outlive a matched realization
without the proposed storage interaction. These two conditions distinguish
memory from a nonzero autocorrelation measured over one finite window.
