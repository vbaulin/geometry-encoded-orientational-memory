...
The same landscape records operation order. With independent Brownian
histories at \(N=256\), two opposed write fields applied as \(AB\) or \(BA\)
give \(M_{\rm ord}=0.634\pm0.020\) for overlapping supports and
\(0.0123\pm0.0027\) for separated supports. At \(h=8k_{\rm B}T\), the sign of
the single-trajectory readout identifies the order in 100\% and
\(55.0\pm1.3\%\) of the two cases, respectively. Their discriminabilities are
\(d'=12.55\pm0.97\) and \(0.31\pm0.05\) across five graphs. A nonzero response
during writing is not sufficient: the histories must remain in different
basins after both fields are removed. The protocol and controls appear in
Supplemental Fig.~S9 and Table~SVI.
