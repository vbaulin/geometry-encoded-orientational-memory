...
state.  Because only \(R\) is measured, we bound the retained information by
the maximum-entropy distribution consistent with it.  On the circle of the
...
\(H=\ln[2\pi I_0(\kappa)]-\kappa I_1(\kappa)/I_0(\kappa)\), so the information
retained per rotor relative to a uniform angle is
\begin{equation}
 \mathcal I=\ln 2\pi-H=
 \kappa\frac{I_1(\kappa)}{I_0(\kappa)}-\ln I_0(\kappa)
 \quad\text{nats.}
 \label{eq:supp-capacity}
\end{equation}
Maximizing the entropy at fixed \(R\) minimizes \(\mathcal I\), so
Eq.~\eqref{eq:supp-capacity} is a lower bound.  It also assumes the angular
error is statistically identical across particles; the archived runs store the
particle-averaged overlap only, so a particle-resolved release trajectory
would replace this bound by a direct estimate.

\begin{table}[ht]
\centering
\caption{Retained information at \(N=1024\), \(J=4\), \(g=5\).  Values are
means over three quenched graphs, with standard deviations obtained by
evaluating Eq.~\eqref{eq:supp-capacity} for each graph separately.}
\label{tab:supp-capacity}
\begin{ruledtabular}
\begin{tabular}{lccc}
state & \(R\) & bits per rotor & bits per layer\\
\hline
written, field applied & \(0.863\pm0.009\) & \(1.481\pm0.051\) & \(1517\pm52\)\\
written, \(D_rt=625\) after removal & \(0.441\pm0.021\) & \(0.296\pm0.029\) & \(303\pm30\)\\
split replicas, \(D_rt=625\) & \(0.462\pm0.010\) & \(0.327\pm0.016\) & \(334\pm16\)\\
\(g=0\) written, \(D_rt=625\) & \(0.006\pm0.004\) & \(7\times10^{-5}\) & \(0.07\)\\
\end{tabular}
\end{ruledtabular}
\end{table}

One fifth of the written target-orientation information survives the
field-free interval, while the matched \(g=0\) control retains none of it.
This is a particlewise information bound for the prepared state under the
present protocol, not a multi-pattern device capacity: it assumes a perfect
particle-resolved readout and a single write.
...
and the last column is the retained information of
Eq.~\eqref{eq:supp-capacity} evaluated on it.}
\label{tab:supp-disorder-retention}
\begin{ruledtabular}
\begin{tabular}{lccccc}
\(\sigma/a\)&graphs&\(S\)&\(Q_{\rm target}\)&\(Q^{\rm conn}\)&bits per rotor\\
\hline
...
motion remains in it. The loop structure therefore controls memory capacity
