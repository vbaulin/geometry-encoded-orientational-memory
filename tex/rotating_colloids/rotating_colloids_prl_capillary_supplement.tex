...
\section{Submission validation protocols}

Three prospective tests address assumptions that are not resolved by the
fixed-graph production runs. They are launched together by
...
The time-step test holds every physical duration fixed while reducing
...
This is a weak-convergence test of graph-averaged observables. Agreement with
the finest step must lie within the larger of the graph standard error and a
predeclared numerical tolerance before a value is promoted into the Letter.
...
accuracy and \(d'\) across five \(N=256\) graphs. This test is required before
the order-of-events construction is interpreted as a passive sensor.
