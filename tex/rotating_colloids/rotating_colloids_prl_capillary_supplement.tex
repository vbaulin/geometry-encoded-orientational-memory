...
One fifth of the written information survives the field-free interval, while
the matched \(g=0\) control retains none of it.  This is a capacity of the
prepared state under the present protocol, not a device specification: it
assumes a perfect particle-resolved readout and a single write.
...
The operating point that maximizes retained hidden memory is therefore the
least positional disorder that still suppresses the director reliably,
\(\sigma\simeq0.11a\).  The value \(\sigma=0.16a\) used throughout this work
...
job, encoded in
\texttt{scripts/run\_rotating\_colloids\_activated\_memory\_prl\_gpu.sh}.
...
\texttt{scripts/run\_rotating\_colloids\_spin\_glass\_prl\_4gpu.sh}, with
\texttt{scripts/run\_rotating\_colloids\_spin\_glass\_prl\_gpu.sh} as the
